...
public class DefaultSlackSettingsService implements SlackSettingsService {
	...
	static final ImmutableSlackSettings DEFAULT_CONFIG = new ImmutableSlackSettings(
			...
			NotificationLevel.VERBOSE, NotificationLevel.VERBOSE, "", // channel
																		// name
																		// override
			"", // webhook override
	...
	static final String KEY_SLACK_USERNAME = "slackUsername";
	static final String KEY_SLACK_ICON_URL = "slackIconUrl";
	static final String KEY_SLACK_ICON_EMOJI = "slackIconEmoji";

	private final PluginSettings pluginSettings;
	private final PermissionValidationService validationService;

	private final Cache<Integer, SlackSettings> cache = CacheBuilder
			.newBuilder().build(new CacheLoader<Integer, SlackSettings>() {
				@Override
				public SlackSettings load(@Nonnull Integer repositoryId) {
					@SuppressWarnings("unchecked")
					Map<String, String> data = (Map) pluginSettings
							.get(repositoryId.toString());
					return data == null ? DEFAULT_CONFIG : deserialize(data);
				}
			});

	public DefaultSlackSettingsService(
			PluginSettingsFactory pluginSettingsFactory,
			PermissionValidationService validationService) {
		this.validationService = validationService;
		this.pluginSettings = pluginSettingsFactory
				.createSettingsForKey(PluginMetadata.getPluginKey());
	}

	@Nonnull
	@Override
	public SlackSettings getSlackSettings(@Nonnull Repository repository) {
		validationService.validateForRepository(
				checkNotNull(repository, "repository"), Permission.REPO_READ);

		try {
			// noinspection ConstantConditions
			SlackSettings settings = cache.get(repository.getId());
			return settings;
	...
	@Nonnull
	@Override
	public SlackSettings setSlackSettings(@Nonnull Repository repository,
			@Nonnull SlackSettings settings) {
		validationService.validateForRepository(
				checkNotNull(repository, "repository"), Permission.REPO_ADMIN);
	...
	// note: for unknown reason, pluginSettngs.get() is not getting back the key
	...
	private Map<String, String> serialize(SlackSettings settings) {
		ImmutableMap<String, String> immutableMap = ImmutableMap
				...
				.put(KEY_SLACK_OVERRIDE_NOTIFICATION,
						Boolean.toString(settings
								.isSlackNotificationsOverrideEnabled()))
				.put(KEY_SLACK_NOTIFICATION,
						Boolean.toString(settings.isSlackNotificationsEnabled()))
				.put(KEY_SLACK_OPENED_NOTIFICATION,
						Boolean.toString(settings
								.isSlackNotificationsOpenedEnabled()))
				.put(KEY_SLACK_REOPENED_NOTIFICATION,
						Boolean.toString(settings
								.isSlackNotificationsReopenedEnabled()))
				.put(KEY_SLACK_UPDATED_NOTIFICATION,
						Boolean.toString(settings
								.isSlackNotificationsUpdatedEnabled()))
				.put(KEY_SLACK_APPROVED_NOTIFICATION,
						Boolean.toString(settings
								.isSlackNotificationsApprovedEnabled()))
				.put(KEY_SLACK_UNAPPROVED_NOTIFICATION,
						Boolean.toString(settings
								.isSlackNotificationsUnapprovedEnabled()))
				.put(KEY_SLACK_DECLINED_NOTIFICATION,
						Boolean.toString(settings
								.isSlackNotificationsDeclinedEnabled()))
				.put(KEY_SLACK_MERGED_NOTIFICATION,
						Boolean.toString(settings
								.isSlackNotificationsMergedEnabled()))
				.put(KEY_SLACK_COMMENTED_NOTIFICATION,
						Boolean.toString(settings
								.isSlackNotificationsCommentedEnabled()))
				.put(KEY_SLACK_NOTIFICATION_PUSH,
						Boolean.toString(settings
								.isSlackNotificationsEnabledForPush()))
				.put(KEY_SLACK_NOTIFICATION_PERSONAL,
						Boolean.toString(settings
								.isSlackNotificationsEnabledForPersonal()))
				.put(KEY_SLACK_NOTIFICATION_LEVEL,
						settings.getNotificationLevel().toString())
				.put(KEY_SLACK_NOTIFICATION_PR_LEVEL,
						settings.getNotificationPrLevel().toString())
				.put(KEY_SLACK_CHANNEL_NAME,
						settings.getSlackChannelName().isEmpty() ? " "
								: settings.getSlackChannelName())
				.put(KEY_SLACK_WEBHOOK_URL,
						settings.getSlackWebHookUrl().isEmpty() ? " "
								: settings.getSlackWebHookUrl())
				.put(KEY_SLACK_USERNAME,
						settings.getSlackUsername().isEmpty() ? " " : settings
								.getSlackUsername())
				.put(KEY_SLACK_ICON_URL,
						settings.getSlackIconUrl().isEmpty() ? " " : settings
								.getSlackIconUrl())
				.put(KEY_SLACK_ICON_EMOJI,
						settings.getSlackIconEmoji().isEmpty() ? " " : settings
								.getSlackIconEmoji()).build();
	...
	private SlackSettings deserialize(Map<String, String> settings) {
        return new ImmutableSlackSettings(
                ...
                !testStringMapValue(settings, KEY_SLACK_USERNAME) ? "" : settings.get(KEY_SLACK_USERNAME).toString(),
                !testStringMapValue(settings, KEY_SLACK_ICON_URL) ? "" : settings.get(KEY_SLACK_ICON_URL).toString(),
                !testStringMapValue(settings, KEY_SLACK_ICON_EMOJI) ? "" : settings.get(KEY_SLACK_ICON_EMOJI).toString()
        );
    }
	
	private boolean testStringMapValue(Map<String, String> settings, String key) {
		...
		if (settings.get(key).toString().equals(" ")) {
			return false;
		}
		return true;
	}
